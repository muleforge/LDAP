...
public class LdapSASLNamespaceHandlerTestCase extends FunctionalTestCase
{

    @Override
    protected String getConfigResources()
    {
        return "ldapsasl-namespace-config.xml";
    }

    public void testNormal() throws Exception
    {
        final LdapSASLConnector c = (LdapSASLConnector) muleContext
                .getRegistry().lookupConnector("ldapConnector1");
        assertNotNull(c);
        assertNotNull(c.getQueries());
        assertTrue(c.getQueries().size() == 2);
        assertNotNull(c.getAttributes());
        assertNotNull(c.getPsFilters());
        assertFalse(c.isUseSSL());
